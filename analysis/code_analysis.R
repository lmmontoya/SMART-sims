...
obj4$A2 = as.character(obj4$A2)
obj4$A2[which(obj4$A2 == "soc")] = "outreach"
obj4$A2 = factor(obj4$A2)
obj4$ID = NULL

options(na.action='na.pass')
data = data.frame(model.matrix(~ ., data = obj4, na.action = "na.fail")[,-1])
...
rule_fun = function(i) {
  ...
  reslist$ltmle.obj_unadj_g0 = suppressMessages(ltmle(data = data[, c("A1soc", "A1voucher", "L_missstatus_beforeT2_ind", "L_outofcare_first_year", "A2navigator", "A2outreach", "A2sms.voucher", "A2stop", "Y")], Anodes = Anodes, Lnodes = c("L_missstatus_beforeT2_ind", "L_outofcare_first_year"), Ynodes = "Y", abar = abar, gform = as.matrix(g0), deterministic.Q.function = det.Q, variance.method = "ic"))
  reslist$ltmle.obj_unadj_gn = suppressMessages(ltmle(data = data[, c("A1soc", "A1voucher", "L_missstatus_beforeT2_ind", "L_outofcare_first_year", "A2navigator", "A2outreach", "A2sms.voucher", "A2stop", "Y")], Anodes = Anodes, Lnodes = c("L_missstatus_beforeT2_ind", "L_outofcare_first_year"), Ynodes = "Y", abar = abar, gform = gform_ADAPT, deterministic.Q.function = det.Q, deterministic.g.function = det.g, variance.method = "ic"))
  reslist$ltmle.obj_adj = suppressMessages(ltmle(data = data, Anodes = Anodes, Lnodes = colnames(data)[grep("L_", colnames(data))], Ynodes = "Y", deterministic.g.function = det.g, deterministic.Q.function = det.Q, abar = abar, SL.library = list(Q = default, g = "glm"), variance.method = "ic", gform = gform_ADAPT)) 
  print(i)
  return(reslist)
...
#save(res, file = "raw_results_analysis.RData")

load(file = "analysis/raw_results_analysis.RData")

# unadj - g0 - IPTW
ind = sapply(1:numrules, function(j) res[[j]]$ltmle.obj_unadj_g0$cum.g.used[,length(Anodes)])
for (rule in 1:numrules) { 
  ind[obj4$A1 == rule.set_ADAPT[rule,1] & data$L_missstatus_beforeT2_ind == 1, rule] = T
}
g0 = sapply(1:numrules, function(j) res[[j]]$ltmle.obj_unadj_g0$cum.g[,length(Anodes)])
H.g0 = ind/g0
psi_IPTW.g0 = sapply(1:numrules, function(j) res[[j]]$ltmle.obj_unadj_g0$estimates[["iptw"]]*mean(H.g0[,j]))
IC_IPTW.g0 = sapply(1:numrules, function(j) H.g0[,j]*data$Y - psi_IPTW.g0[j])

# adj - gn - TMLE
psi_TMLE.unadj = sapply(1:numrules, function(j) res[[j]]$ltmle.obj_unadj_gn$estimates[["tmle"]])
IC_TMLE.unadj = sapply(1:numrules, function(j) res[[j]]$ltmle.obj_unadj_gn$IC$tmle)

# adj - gn - TMLE
psi_TMLE.adj = sapply(1:numrules, function(j) res[[j]]$ltmle.obj_adj$estimates[["tmle"]])
IC_TMLE.adj = sapply(1:numrules, function(j) res[[j]]$ltmle.obj_adj$IC$tmle)
...
for_return_fun = function(psi, IC, name) {
  ...
  toreturn = list(psi = psi, CI_ind = CI_ind, CI_simult = CI_simult, IC = IC)
...
results_adj_ADAPT = for_return_fun(psi = psi_TMLE.adj, IC = IC_TMLE.adj, name = "TMLE.adj")
results_unadj_ADAPT = for_return_fun(psi = psi_TMLE.unadj, IC = IC_TMLE.unadj, name = "TMLE.unadj")

save(results_adj_ADAPT, results_unadj_ADAPT, file = "analysis/results_analysis.RData")
