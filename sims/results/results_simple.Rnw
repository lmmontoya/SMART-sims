...
write.csv(truth_simple, file = "sims/truths_simple.csv", row.names = F)
load(file = "sims/results/simresults_simple.RData")
truth = truth_simple
rm(truth_realcovs, truth_simple)
...
<<echo = F, results = 'asis'>>=
est = c("IPTW.g0", "IPTW.gn.unadj", "IPTW.gn.adj", "Gcomp.gn.adj", "TMLE.gn.adj")
...
cov_ind_df = tidyr::gather(data.frame(cov_ind), "rule", "cov_ind", 1:8)
...
bias_df = tidyr::gather(data.frame(bias), "rule", "bias", 1:8)
bias_df$rule = as.factor(substr(bias_df$rule, start = 2, stop = nchar(bias_df$rule)))
bias_df$rule = as.factor(as.numeric(bias_df$rule))
bias_df$Estimator = factor(bias_df$Estimator, levels = c("IPTW.g0", "IPTW.gn.unadj", "IPTW.gn.adj", "Gcomp.gn.adj", "TMLE.gn.adj"))
bias = bias_df %>%
 ...
 # xlab("Embedded Regime #") +
  scale_shape_manual(name = "Estimator", values = c(1,2,3,4,5), labels = c("IPTW.g0" = parse(text = TeX('Min. adj. IPW w/ $g_0$')),
                                                                    ...
                                                                    "TMLE.gn.adj" = 'Full adj. TMLE',
                                                                    "Gcomp.gn.adj" = 'Full adj. G-comp.'))
bias


var_df = tidyr::gather(data.frame(var), "rule", "var", 1:8)
...
var_df$Estimator = factor(var_df$Estimator, levels = c("IPTW.g0", "IPTW.gn.unadj", "IPTW.gn.adj", "TMLE.gn.adj"))
var = var_df %>%
...
ci_width_df = tidyr::gather(data.frame(ci_width), "rule", "ci_width", 1:8)
...
ci_width_df$Estimator = factor(ci_width_df$Estimator, levels = c("IPTW.g0", "IPTW.gn.unadj", "IPTW.gn.adj", "TMLE.gn.adj"))
ci = ci_width_df %>%
...
png(filename="sims/results/plot_sim_simple.png", width = 700, height = 500)
leg = get_legend(bias)
...
annotate_figure(figure, bottom = text_grob("Embedded Regime #", size = 16), top = text_grob("Simulation Performance Results: DGP 1",  size = 20))
#figure
...
#ggarrange(dp, dp, dp,  leg, ncol = 2, nrow = 2)
...
df$Estimator = recode(df$Estimator, 
                      ...
                      TMLE.gn.adj = "Full adj. TMLE")
write.csv(df, file = "sims/results/results_simple.csv", row.names = F)
@

Calcs
<<echo = F>>=
range(df$Bias[df$Estimator == "Full adj. G-comp."])*100
range(lapply(1:8, function(i) df$Bias[df$Estimator == "Full adj. G-comp." & df$Rule == i]/df$Bias[df$Estimator != "Full adj. G-comp." & df$Rule == i]))

range(lapply(1:8, function(i) df$Var.[df$Estimator == "Min. adj. IPW (w/ $g_0$)" & df$Rule == i]/df$Var.[df$Estimator != "Min. adj. IPW (w/ $g_0$)" & df$Rule == i]), na.rm = T)
...
range(lapply(1:8, function(i) df$Var.[df$Estimator != "Full adj. TMLE" & df$Rule == i]/df$Var.[df$Estimator == "Full adj. TMLE" & df$Rule == i]), na.rm = T)
range(lapply(1:8, function(i) df$`C.I. Width`[df$Estimator != "Full adj. TMLE" & df$Rule == i]/df$`C.I. Width`[df$Estimator == "Full adj. TMLE" & df$Rule == i]), na.rm = T)
